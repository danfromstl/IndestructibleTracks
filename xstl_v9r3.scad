/*
    ...
    V9R3 goals:
      - Preserve the working V9R2 geometry and controls.
      - Repair normal F5 display of complex flexy tracks by resolving the
        completed low-resolution preview to a mesh before OpenCSG displays it.
      - Keep F6 render and STL/3MF export on the unchanged raw geometry path.
      ...
      - Use the newer 24-oscillation, 0.8 mm gap flexy defaults.
      - Keep flexy rail wells continuous, but add simple 45-degree chamfers
        to their mouth, walls, and floor.
      - Add a simple 45-degree plan-view chamfer to the inner relief-gap
        corners instead of the abandoned rounded-slot experiment.
...
// The flexy track has many nested subtraction branches. OpenCSG can omit the
// body in normal F5 view even though the CSG tree is valid (it remains visible
// in Thrown Together and renders correctly with F6). When enabled, V9R3 uses
// render() only during F5 so OpenSCAD displays a resolved preview mesh.
// Disable only for diagnosing the raw OpenCSG tree.
preview_safe_flexy = true;

// Convexity is only a preview ray-depth hint; it does not change mesh detail.
preview_flexy_convexity = 256;           // [32:32:512]
...
flexy_start_block_length = 34.30;      // [15:0.1:80]

// Solid body length at local finish. With defaults, this is the plug-end block.
flexy_finish_block_length = 16.10;     // [10:0.1:80]
...
flexy_gap_length = 0.80;               // [0.4:0.05:8]

// Uncut side bridge left across each relief gap.
flexy_bridge_width = 13.00;            // [1:0.05:18]
...
assert(preview_flexy_convexity >= 1,
       "preview_flexy_convexity must be at least 1");
...
module flexy_top_track_well_cutters_2d() {
    for (x_offset = [-track_well_spacing / 2,
                      track_well_spacing / 2]) {
        translate([
            track_width / 2 + x_offset,
...
module flexy_bottom_track_well_cutters_2d() {
    for (x_offset = [-track_well_spacing / 2,
                      track_well_spacing / 2]) {
        translate([
            track_width / 2 + x_offset,
...
module flexy_track_well_cutters_2d(include_bottom_wells = two_sided) {
    flexy_top_track_well_cutters_2d();

    if (include_bottom_wells)
        flexy_bottom_track_well_cutters_2d();
}
...
// V9R2's original flexy boolean tree, kept unchanged for F6/export.
module flexy_straight_track_raw() {
    difference() {
        ...
        flexy_rail_well_cutters_3d(flexy_body_length, two_sided);
        flexy_relief_gap_cutters();
    }
}


// OpenCSG can fail to display the deeply nested flexy difference() during F5.
// Resolving only the low-resolution preview to a mesh makes normal Preview
// reliable. Because $preview remains true inside render(), the preview $fn is
// still used. F6/export bypasses this wrapper and preserves the raw V9R2 path.
module flexy_straight_track() {
    if ($preview && preview_safe_flexy) {
        render(convexity = preview_flexy_convexity)
...
if (show_diagnostics) {
    ...
    if (track_type == "flexy_straight") {
        echo("F5 preview-safe flexy mesh / convexity:",
             preview_safe_flexy, preview_flexy_convexity);
        ...
        echo("Flexy relief-gap chamfer setting / effective run (mm):",
             body_chamfer, body_chamfer_run);
        echo("Flexy inner corner chamfer effective run (mm):",
             flexy_inner_corner_chamfer_run);
