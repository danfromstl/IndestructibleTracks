/*
    ...
    It preserves V9R3 geometry while simplifying the Customizer, preview
    routing, diagnostics, and repeated rail-well helpers.
*/
...
flexy_start_end = "nest";  // [plug:Plug,nest:Nest,none:None]
flexy_finish_end = "plug"; // [plug:Plug,nest:Nest,none:None]

// Solid body beside the start and finish connectors.
flexy_start_block_length = 24.30;  // [10:0.1:80]
flexy_finish_block_length = 12.00; // [10:0.1:80]

// Repeated solid sections; gap count is oscillations + 1.
flexy_oscillations = 48; // [1:1:60]

// Solid-section length and relief-gap width along the track axis.
flexy_section_length = 2.20; // [1:0.05:20]
flexy_gap_length = 0.50;     // [0.4:0.05:8]

// Uncut side bridge left across each alternating relief gap.
flexy_bridge_width = 13.00; // [1:0.05:18]
flexy_gap_bridge_phase = "right_first"; // [right_first:Right first,left_first:Left first]
...
module track_well_cutters_2d(
    ...
    for (x_offset = [-track_well_spacing / 2,
        ...
        translate([x, track_height - track_well_depth + epsilon])
            selected_rail_well_cutter_2d(flexy_profile);

        if (include_bottom_wells)
            translate([x, track_well_depth - epsilon])
                rotate(180)
                    selected_rail_well_cutter_2d(flexy_profile);
    }
}
...
// V9R3 flexy geometry, unchanged apart from the shared well-placement helper.
module flexy_straight_track_raw() {
    difference() {
        ...
        rail_well_cutters_3d(flexy_body_length, two_sided, true);
...
module console_full_report() {
    ...
    if (track_type == "flexy_straight") {
        ...
        echo(str(
            ...
            " mm | outer shoulder=", flexy_outer_shoulder_width, " mm"
        ));
    }
