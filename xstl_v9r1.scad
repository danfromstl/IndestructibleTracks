/*
    ...
    V9R1 goals:
      ...
      - Apply body-style chamfers to the flexy relief-gap cut edges.
      - Replace the flexy middle rail wells with rounded, chamfered slot
        cutouts so the well ends, walls, and floors soften like the body
        edges.
...
flexy_start_block_length = 34.10;      // [15:0.1:80]

// Solid body length at local finish. With defaults, this is the plug-end block.
flexy_finish_block_length = 20.10;     // [15:0.1:80]
...
flexy_bridge_width = 12.00;            // [1:0.05:18]
...
flexy_well_slot_end_margin =
    min(body_chamfer_run / 2, flexy_section_length / 4);
flexy_well_slot_length =
    max(epsilon, flexy_section_length - 2 * flexy_well_slot_end_margin);
flexy_well_chamfer_run = max(0, min(
    min(body_chamfer_run, track_well_depth / 2 - epsilon),
    min(track_well_width_bottom / 2 - epsilon,
        flexy_well_slot_length / 2 - epsilon)
));
flexy_well_curve_fragments =
    $preview ? min(preview_curve_fragments, 16)
             : min(render_curve_fragments, 24);
...
assert(flexy_well_slot_length > 0,
       "flexy_section_length is too short for segmented rail-well slots");
...
// Rounded rectangle in X/Y, centered on the local origin.
module rounded_rectangle_2d(length, width) {
    safe_length = max(epsilon, length);
    safe_width = max(epsilon, width);
    radius = min(safe_length, safe_width) / 2;

    if (safe_length >= safe_width) {
        center_length = max(epsilon, safe_length - safe_width);

        union() {
            square([safe_width, center_length], center = true);

            for (y = [-center_length / 2, center_length / 2]) {
                translate([0, y])
                    circle(r = radius, $fn = flexy_well_curve_fragments);
            }
        }
    } else {
        center_width = max(epsilon, safe_width - safe_length);

        union() {
            square([center_width, safe_length], center = true);

            for (x = [-center_width / 2, center_width / 2]) {
                translate([x, 0])
                    circle(r = radius, $fn = flexy_well_curve_fragments);
            }
        }
...
function flexy_section_start_y(section_index) =
    flexy_gap_start_y(section_index) + flexy_gap_length;


module flexy_straight_top_rail_well_cutters_between(y_min, y_max) {
    extrude_xz_profile_between_y(y_min - epsilon, y_max + epsilon)
        translate([-track_width / 2, 0])
            top_track_well_cutters_2d();
}


module flexy_straight_bottom_rail_well_cutters_between(y_min, y_max) {
    extrude_xz_profile_between_y(y_min - epsilon, y_max + epsilon)
        translate([-track_width / 2, 0])
            bottom_track_well_cutters_2d();
}
...
module flexy_rail_well_slot_pattern_2d(slot_length, slot_width) {
    for (section_index = [0 : flexy_oscillations - 1]) {
        for (x_offset = [-track_well_spacing / 2,
                          track_well_spacing / 2]) {
            translate([
                x_offset,
                flexy_section_start_y(section_index)
                    + flexy_well_slot_end_margin
                    + flexy_well_slot_length / 2
            ])
                rounded_rectangle_2d(slot_length, slot_width);
        }
    }
}


module flexy_segmented_top_rail_well_layer(
    slot_length,
    slot_width,
    z_min,
    z_max
    ...
    translate([0, 0, z_min])
        linear_extrude(
            height = max(epsilon, z_max - z_min),
            convexity = 10
        )
            flexy_rail_well_slot_pattern_2d(
                max(epsilon, slot_length),
                max(epsilon, slot_width)
            );
}


module flexy_segmented_top_rail_well_cutters() {
    run = flexy_well_chamfer_run;
    has_chamfer = body_chamfer > 0 && run > 0;
    z_floor = track_height - track_well_depth - epsilon;
    z_mouth = track_height + epsilon;

    if (has_chamfer) {
        flexy_segmented_top_rail_well_layer(
            flexy_well_slot_length - 2 * run,
            track_well_width_bottom - 2 * run,
            z_floor,
            z_mouth
        );
        flexy_segmented_top_rail_well_layer(
            flexy_well_slot_length,
            track_well_width_top,
            z_floor + run,
            z_mouth
        );
        flexy_segmented_top_rail_well_layer(
...
module flexy_segmented_bottom_rail_well_cutters() {
    translate([0, 0, track_height])
        mirror([0, 0, 1])
            flexy_segmented_top_rail_well_cutters();
}


module flexy_rail_well_cutters() {
    flexy_straight_top_rail_well_cutters_between(
        0,
        flexy_start_block_length
    );
    flexy_straight_top_rail_well_cutters_between(
        flexy_body_length - flexy_finish_block_length,
        flexy_body_length
    );
    flexy_segmented_top_rail_well_cutters();

    if (two_sided) {
        flexy_straight_bottom_rail_well_cutters_between(
            0,
            flexy_start_block_length
        );
        flexy_straight_bottom_rail_well_cutters_between(
        ...
        flexy_segmented_bottom_rail_well_cutters();
    }
}
...
module flexy_y_edge_chamfer_cutter(
    ...
    if (body_chamfer > 0 && run > 0) {
        if (material_y_direction < 0) {
            extrude_yz_profile_between_x(x_min, x_max)
                polygon([
                    [gap_edge_y - run, track_height + epsilon],
                    [gap_edge_y + epsilon, track_height + epsilon],
                    [gap_edge_y + epsilon,
            ...
            extrude_yz_profile_between_x(x_min, x_max)
                polygon([
                    [gap_edge_y - run, -epsilon],
                    [gap_edge_y + epsilon, -epsilon],
                    [gap_edge_y + epsilon, run + epsilon]
                ]);
        } else {
            extrude_yz_profile_between_x(x_min, x_max)
                polygon([
...
module flexy_x_edge_chamfer_cutter(
    ...
    if (body_chamfer > 0 && run > 0) {
        if (material_x_direction < 0) {
            extrude_xz_profile_between_y(y_min, y_max)
                polygon([
                    [bridge_edge_x - run, track_height + epsilon],
                    [bridge_edge_x + epsilon, track_height + epsilon],
                    [bridge_edge_x + epsilon,
                     track_height - run - epsilon]
                ]);

            extrude_xz_profile_between_y(y_min, y_max)
                polygon([
                    [bridge_edge_x - run, -epsilon],
                    [bridge_edge_x + epsilon, -epsilon],
                    [bridge_edge_x + epsilon, run + epsilon]
                ]);
        } else {
            extrude_xz_profile_between_y(y_min, y_max)
                polygon([
...
if (show_diagnostics) {
    ...
    if (track_type == "flexy_straight") {
        ...
        echo("Flexy relief-gap chamfer setting / effective run (mm):",
             body_chamfer, body_chamfer_run);
        echo("Flexy segmented rail-well slot length / end margin (mm):",
             flexy_well_slot_length, flexy_well_slot_end_margin);
        echo("Flexy rail-well chamfer effective run (mm):",
