/*
    ...
    V11 targets current OpenSCAD nightly builds with the Manifold backend.
    ...
    reference STL named "Adapted Original - SF200.stl". V11 keeps the
    rounded through-cut relief slots, adds continuous top/bottom relief
    chamfers, and uses explicit flex-body end chamfers to avoid end-mask
    artifacts around the first and last track-well features.
*/
...
// Extrude a child X/Y profile between two Z coordinates.
module extrude_xy_profile_between_z(z_min, z_max, convexity = 10) {
    assert(z_max > z_min, "z_max must be greater than z_min");

    translate([0, 0, z_min])
        linear_extrude(height = z_max - z_min, convexity = convexity)
            children();
}
...
module flexy_relief_edge_chamfer_profiles_2d(
    edge,
    material_direction
) {
...
module flexy_relief_y_edge_chamfer_cutter(
    gap_edge_y,
    x_min,
    x_max,
    material_y_direction
) {
    if (flexy_relief_edge_chamfer_run > 0)
        extrude_yz_profile_between_x(x_min, x_max)
            flexy_relief_edge_chamfer_profiles_2d(
                gap_edge_y,
                material_y_direction
            );
}
...
module flexy_rounded_relief_cap_chamfer_cutter(
    cap_x,
    center_y,
    bridge_on_right
) {
    run = flexy_relief_edge_chamfer_run;
    inner_radius = flexy_gap_length / 2 + flexy_gap_y_overcut;
    surface_radius = inner_radius + run;
    start_rotation = bridge_on_right ? -90 : 90;

    if (run > 0)
        translate([cap_x, center_y, 0])
            rotate([0, 0, start_rotation]) {
                rotate_extrude(angle = 180, convexity = 10)
                    polygon([
                        [inner_radius - epsilon,
                         track_height - run - epsilon],
                        [surface_radius + epsilon,
                         track_height + epsilon],
                        [inner_radius - epsilon,
                         track_height + epsilon]
                    ]);
...
module flexy_rounded_relief_gap_chamfer_cutters(
    gap_start_y = flexy_start_block_length,
    bridge_on_right = true
) {
    inner_radius = flexy_gap_length / 2 + flexy_gap_y_overcut;
    center_y = gap_start_y + flexy_gap_length / 2;
    y_min = center_y - inner_radius;
    y_max = center_y + inner_radius;
    cap_x = bridge_on_right
        ? track_width / 2 - flexy_bridge_width
        : -track_width / 2 + flexy_bridge_width;
    ...
    x_max = bridge_on_right
        ? cap_x + epsilon
        : track_width / 2 + epsilon;

    flexy_relief_y_edge_chamfer_cutter(
        y_min,
        x_min,
        x_max,
        -1
    ...
    flexy_rounded_relief_cap_chamfer_cutter(
        cap_x,
        center_y,
        bridge_on_right
    );
...
module flexy_rounded_relief_gap_cutter(
    ...
    if (run > 0) {
        ...
        flexy_rounded_relief_gap_chamfer_cutters(
            gap_start_y,
            bridge_on_right
        );
    } else {
...
function flexy_end_side_chamfer_triangle(
    edge_y,
    material_y_direction,
    edge_x,
    material_x_direction,
    ...
    [
        edge_x - material_x_direction * epsilon,
        edge_y - material_y_direction * epsilon
    ],
    [
        edge_x - material_x_direction * epsilon,
        edge_y + material_y_direction * (run + epsilon)
    ],
    [
        edge_x + material_x_direction * (run + epsilon),
        edge_y - material_y_direction * epsilon
    ]
];


module flexy_end_z_chamfer_cutter(
    edge_y,
    material_y_direction,
    surface_z,
    inner_z
...
module flexy_end_side_chamfer_cutter(
    edge_y,
    material_y_direction,
    edge_x,
    material_x_direction
    ...
    if (track_end_chamfer > 0 && run > 0)
        extrude_xy_profile_between_z(
            -epsilon,
            track_height + epsilon
        )
            polygon(flexy_end_side_chamfer_triangle(
                edge_y,
                material_y_direction,
                edge_x,
                material_x_direction,
...
module flexy_body_end_chamfer_cutters(length = flexy_body_length) {
    for (end = [
        [0, 1],
        [length, -1]
    ]) {
        edge_y = end[0];
        material_y_direction = end[1];

        flexy_end_z_chamfer_cutter(
            edge_y,
            material_y_direction,
            track_height + epsilon,
            track_height - track_end_chamfer_run - epsilon
        );
        flexy_end_z_chamfer_cutter(
            edge_y,
            material_y_direction,
            -epsilon,
            track_end_chamfer_run + epsilon
        );
        flexy_end_side_chamfer_cutter(
            edge_y,
            material_y_direction,
            -track_width / 2,
            1
...
module flexy_body_with_explicit_end_chamfers(
    length = flexy_body_length
) {
    difference() {
        straight_body_raw(length, false, false);
        flexy_body_end_chamfer_cutters(length);
    }
}


module flexy_body_with_sf200_cutters(length = flexy_body_length) {
    difference() {
        straight_body_raw(length, false, false);
        flexy_body_end_chamfer_cutters(length);
...
module flexy_track_length(
    length = flexy_body_length,
    start_connector = flexy_start_end,
    finish_connector = flexy_finish_end
) {
    difference() {
        union() {
            flexy_body_with_sf200_cutters(length);
